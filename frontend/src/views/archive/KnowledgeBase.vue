...
const loadKnowledgeList = async () => {
  try {
    ...
    const response = await archiveApi.searchKnowledge(params)
...
const loadStatistics = async () => {
  try {
    ...
    const response = await archiveApi.searchKnowledge({ limit: 1000 })
...
const handleDelete = async (item: any) => {
  try {
    ...
    await archiveApi.deleteKnowledge(item.id)
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (isEdit.value && formData.id) {
        await archiveApi.updateKnowledge(formData.id, formData)
        ElMessage.success('更新成功')
      } else {
        await archiveApi.createKnowledge(formData)
