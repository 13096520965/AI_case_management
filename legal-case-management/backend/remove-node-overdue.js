...
/**
 * 删除 node_overdue 类型的提醒数据
 */
async function removeNodeOverdueNotifications() {
  return new Promise((resolve, reject) => {
    ...
    console.log('开始清除 node_overdue 类型的提醒数据...\n');
    ...
    db.serialize(() => {
      ...
      `, (err, rows) => {
        ...
        console.log(`找到 ${rows.length} 条 node_overdue 类型的数据:\n`);
        rows.forEach((row, index) => {
          console.log(`${index + 1}. [ID: ${row.id}] ${row.content}`);
        });
        console.log('');
        ...
        `, function(err) {
          ...
          console.log(`✓ 成功删除 ${this.changes} 条 node_overdue 类型的数据\n`);
...
async function main() {
  try {
    ...
    console.log('清除 node_overdue 类型提醒数据');
    console.log('='.repeat(60));
    console.log('');

    await removeNodeOverdueNotifications();
