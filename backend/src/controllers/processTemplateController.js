...
exports.getTemplates = async (req, res) => {
  try {
    ...
    // 为每个模板添加节点数量
    const templatesWithNodeCount = await Promise.all(
      templates.map(async (template) => {
        const nodes = await ProcessTemplateNode.findByTemplateId(template.id);
        return {
          ...template,
    ...
    res.json({
      data: {
        templates: templatesWithNodeCount
      }
    });
  } catch (error) {
 ...
 * 获取流程模板的节点列表
 */
exports.getTemplateNodes = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // 获取模板节点
    const nodes = await ProcessTemplateNode.findByTemplateId(id);
